...
public class ValidarCampos {
    ...
    public static boolean ValidarCPF(View pView) {
        if (pView instanceof EditText) {
            ...
            if (text != null) {
                ...
                if (!CPF.equals("00000000000") & !CPF.equals("11111111111") & !CPF.equals("22222222222") & !CPF.equals("33333333333")
                    ...
                    try {
                        ...
                        edText.setError("Esse CPF não existe, verifique novamente");
                        edText.setFocusable(true);
                        edText.requestFocus();
                        return (false);
                    }
            ...
            edText.setError("Seu CPF não foi digitado na forma correta");
            edText.setFocusable(true);
            edText.requestFocus();
            return false;
        }
    ...
    public final static boolean ValidarEmail(View pView) {
        if (pView instanceof EditText) {
            ...
            edText.setError("Seu Email não foi digitado na forma correta");
            edText.setFocusable(true);
            edText.requestFocus();
            return false;
        }
    ...
    public final static boolean ValidarPassword(View pView) {
        if (pView instanceof EditText) {
            ...
            if (text != null) {
                ...
                boolean temespecial = pe.matcher(password).find();   //caracteres especiais
                boolean temmaiuscula = pm.matcher(password).find(); // Letras Maiúsculas
                boolean temnumero = pn.matcher(password).find();    // numeros

                if (temespecial & temmaiuscula & temnumero) {
                    return true;
                }
            }
            edText.setError("O Password precisa ter uma letra Maiuscula, um caracter Especial e um Numero");
            edText.setFocusable(true);
            edText.requestFocus();
            return false;
        }
